...
public class MD5 {
    ...
    */
    ...
    private String changeHex(int a) {
        String str = "";
        for (int i = 0; i < 4; i++) {
            str += String.format("%2s", Integer.toHexString((int) (((a >> i * 8) % (1 << 8)) & 0xff))).replace(' ', '0');
        }
        return str;
    }
    ...
    */
     ...
     * 计算文件的MD5摘要
     */
    public String getMD5_file(String path) {
        ...
        if (file.exists()) {
            try {
                ...
                while ((originalText = originalTextWriter.readLine()) != null) {
                    init();
                    int strByte[] = extend(originalText);
                    for (int i = 0; i < strByte.length / 16; i++) {
                        int num[] = new int[16];
                        for (int j = 0; j < 16; j++) {
                            num[j] = strByte[i * 16 + j];
